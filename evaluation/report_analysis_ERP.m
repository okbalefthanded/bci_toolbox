...
fprintf(fid, 'Subject | Train Accuracy | Test Accuray | Max correct | N° Trials | ITR_max | ITR_min \n');

for subj = 1:length(results)
    fprintf(fid, 'S0%d  %f  %f  %f  %f %f %f \n\n', subj,... 
            results(subj).train_acc, results(subj).test_acc,...
            results(subj).min_subject_sequence(1),... 
            results(subj).min_subject_sequence(2),...
            results(subj).max_itr,...
            results(subj).min_itr);
end
