function [model] = ml_trainSIMPLEMKL(features, alg, cv)
%ML_TRAINSIMPLEMKL Summary of this function goes here
...
if(isempty(alg.options))
    alg.options.parameters = smksvm_parameter();
end
...
if(cv.nfolds == 0)
    if(strcmp(classMode,'Bin'))
        ...
        models = smksvm_train(train_data, alg.options.parameters);
    else        
        for m =1:nModels
            ft = features;            
            ...
            models{m} = smksvm_train(train_data, alg.options.parameters);
            clear ft
        end
    end        
    model.model = models;
    model.alg.learner = 'SIMPLEMKL';
