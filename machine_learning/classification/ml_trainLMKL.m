function [model] = ml_trainLMKL(features, alg, cv)
%ML_TRAINLMKL Summary of this function goes here
...
if(isempty(alg.options))
    alg.options.parameters = lmksvm_parameter();
end
...
if(cv.nfolds == 0)
    if(strcmp(classMode,'Bin'))
        ...
        models = lmksvm_train(train_data, alg.options.parameters);
    else        
        for m =1:nModels
            ft = features;            
            ...
            models{m} = lmksvm_train(train_data, alg.options.parameters);
            clear ft
        end
    end        
    model.model = models;
    model.alg.learner = 'LMKL';
...
if(isfield(alg,'o'))
    ...
    [alg.('options').('parameters').('gat').('typ')] = alg.o.p.g.t;
    [alg.('options').('parameters').('loc').('typ')] = alg.o.p.l.t;
    [alg.('options').('parameters').('nor').('loc')] = alg.o.p.n.l;
    [alg.('options').('parameters').('see')] = alg.o.p.s;
    fields = {'o'};
end
if(isfield(cv, 'n'))
    [cv.('nfolds')] = cv.('n');    
