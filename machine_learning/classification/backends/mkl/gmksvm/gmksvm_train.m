...
%   Manic Varma, Bodla Rakesh Babu
%   Proceedings of the 26th International Conference on Machine Learning, 2009

% Summary
%   trains GMKSVM on training data with given parameters
...
function mod = gmksvm_train(tra, par)
    P = length(tra);
    for m = 1:P
...
function [alp, eta, mod, obj, yyKeta] = learn_eta(tra, par, yyKm, alp, eta, mod, obj, yyKeta)    
    P = length(tra);
    sup = find(alp ~= 0);
    gra = eta_gradient_gmk(alp(sup), yyKm(sup, sup, :)) + par.sig * (eta - ones(1, P) / P) * eye(P, P);
    srn = sqrt(sum(gra.^2));
    if srn ~= 0
        gra = gra ./ srn;
    else
