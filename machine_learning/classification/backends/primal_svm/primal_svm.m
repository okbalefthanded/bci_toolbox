function [sol,b,obj] = primal_svm(X,Y,linear,lambda,opt)
%   ---[SOL, B] = PRIMAL_SVM(LINEAR,Y,LAMBDA,OPT)
% [SOL, B] = PRIMAL_SVM(X,Y,LINEAR,Y,LAMBDA,OPT)
...
%     global K;
    if isempty(X), error('Global variable X undefined'); end;
    n = size(Y,1);
    if any(size(X)~=n), error('Dimension error'); end;
    if ~opt.cg 
      [sol,obj] = primal_svm_nonlinear   (X,Y,lambda,opt); 
    else
      [sol,obj] = primal_svm_nonlinear_cg(X,Y,lambda,opt); 
    end;
...
%   global X;
  ...
  while 1
    ...
    [obj, grad, sv] = obj_fun_linear(X,w,Y,lambda,out);      
    ...
    [t,out] = line_search_linear(X,w,step,out,Y,lambda);
...
function [beta,obj] = primal_svm_nonlinear(X,Y,lambda,opt)
% -----------------------------------
...
%   global X
  ...
  if n>=1000  % Train a subset first
    ...
    Y2 = Y; Y2(ind) = 0;
    beta = primal_svm_nonlinear(X,Y2,lambda,opt);
    sv = find(beta(1:end-1)~=0);
    Kb = X(training,sv)*beta(sv);  % Kb will always contains K times the current beta
  else
    sv = training;
  ...
  while 1 
    ...
    H = X(sv,sv) + lambda*eye(length(sv));
    cte_for_b = mean(diag(X));
    ...
    step = beta_new - beta; 
    [t, Kb] = line_search_nonlinear(X, step([training; end]),Kb,beta(end),Y,lambda,1);
...
function  [beta, obj] = primal_svm_nonlinear_cg(X,Y,lambda,opt)
% -----------------------------------------------------
% Train a linear SVM using nonlinear conjugate gradient 
% -----------------------------------------------------
%   global K;
  ...
  Kgo = [X*Y; sum(Y)]; % We use the preconditioner [[K 0]; [0 1]]
  ...
  while 1
    ...
    % Do an exact line search
    [t,Kb] = line_search_nonlinear(X,s,Kb,beta(end),Y,lambda,0,Ks);
    beta = beta + t*s;
    ...
    Kgn = [X*gn(1:end-1); gn(end)];  % Multiply by the preconditioner 
...
function [t, Kb] = line_search_nonlinear(X, step,Kb,b,Y,lambda,fullstep,Ks)
...
%   global K;
  ...
  if nargin<8
    Ks = X(training,training(act))*step(act);
  end; 
