function [model] = ml_trainRBMKL(features, alg, cv)
%ML_TRAINRBMKL Summary of this function goes here
...
if(isempty(alg.options))
    alg.options.parameters = rbmksvm_parameter();
end
...
if(cv.nfolds == 0)
    if(strcmp(classMode,'Bin'))
        ...
        models = rbmksvm_train(train_data, alg.options.parameters);
    else        
        for m =1:nModels
            ft = features;            
            ...
            models{m} = rbmksvm_train(train_data, alg.options.parameters);
            clear ft
        end
    end        
    model.model = models;
    model.alg.learner = 'RBMKL';
...
if(isfield(alg,'o'))
    ...
    [alg.('options').('parameters').('rul')] = alg.o.p.('r');
    fields = {'o'};
end
if(isfield(cv, 'n'))
    [cv.('nfolds')] = cv.('n');
...
alg.o.p.r = alg.options.parameters.rul;
