function [model] = ml_trainGMKL(features, alg, cv)
%ML_TRAINGMKL Summary of this function goes here
...
if(isempty(alg.options))
    alg.options.parameters = gmksvm_parameter();
end
...
if(cv.nfolds == 0)
    if(strcmp(classMode,'Bin'))
        ...
        models = gmksvm_train(train_data, alg.options.parameters);
    else        
        for m =1:nModels
            ft = features;            
            ...
            models{m} = gmksvm_train(train_data, alg.options.parameters);
            clear ft
        end
    end        
    model.model = models;
    model.alg.learner = 'GMKL';
...
if(isfield(alg,'o'))
    ...
    [alg.('options').('parameters').('sig')] = alg.o.p.('s');
...
%TODO
alg.o.p.s = alg.options.parameters.sig; 
m = 1;
n = 1;
off = 0;
for i=1:nWorkers
