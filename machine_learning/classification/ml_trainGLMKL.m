function [ model ] = ml_trainGLMKL(features, alg, cv)
%ML_TRAINGLMKL Summary of this function goes here
...
if(isempty(alg.options))
    alg.options.parameters = glmksvm_parameter();
end
...
if(cv.nfolds == 0)
    if(strcmp(classMode,'Bin'))
        ...
        models = glmksvm_train(train_data, alg.options.parameters);
    else        
        for m =1:nModels
            ft = features;            
            ...
            models{m} = glmksvm_train(train_data, alg.options.parameters);
            clear ft
        end
    end        
    model.model = models;
    model.alg.learner = 'GLMKL';
