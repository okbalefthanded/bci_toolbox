function [model] = ml_trainCABMKL(features, alg, cv)
...
if(isempty(alg.options))
    alg.options.parameters = cabmksvm_parameter();
end
...
if(cv.nfolds == 0)
    if(strcmp(classMode,'Bin'))
        ...
        models = cabmksvm_train(train_data, alg.options.parameters);
    else        
        for m =1:nModels
            ft = features;            
            ...
            models{m} = cabmksvm_train(train_data, alg.options.parameters);
            clear ft
        end
    end        
    model.model = models;
    model.alg.learner = 'CABMKL';
